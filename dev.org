...
*** Moving the Canvas
		...
		Zoom is a [[https://en.wikipedia.org/wiki/Homothetic_transformation][homothetic operation]].
...
**** Solution [2017-08-30 Wed 20:00]
		 So 2 months of putting this off between 4 half assed attempts and I now
		 have a solution.

		 It's critical to realise that there are 3 spaces we're dealing with.
...
*** Musings [2017-08-23 Wed 12:05]
		I'm getting more and more convinced that the representations of shapes from
		the ground up as layered compositions of affine transformations of shapes is
		the way to go. It has both the logical simplicity of a good proof and the
		hierarchical nature of visual chunking which could make it easier to
		represent shapes the way people perceive them and thus it might be easier to
		teach the computer to do "the right thing". The right thing really just
		means responding in a way that doesn't surprise the user, or surprises them
		in a creative way. If surprised they should laugh or say aha! Otherwise the
		computer is wrong. Great definition. Go home.

		The html canvas object itself lets me set the affine transformation. That's
		saves me an immense amount of work. I wonder if that will stunt the
		algorithm's learning at all?

		Using canvas atxs, I don't need any of the linear algebra logic I just set
		up. I can go back to using numbers and clj vectors for scalars and
		vectors. I don't think I need any special semantics...

** Mandelbrot Demo
	 Idea for a lazy data structure that represents the whole of R².

	 Just use the grid. Bitree with neighbours (8 per grid) and children which are
	 subgrids. At any given time only 4 grids will be on screen and the position
	 and zoom level can be used to pick which ones are necessary. If we keep the
	 whole tree in memory then it will expand in memory as we navigate around and
	 the lazy-seqs get realised. Nota big deal for a demo. Alternately we could
	 generate a data structure that drops doesn't retain any connections to its
	 neighbours but each node of the grid has a lazy seq of neighbours, going back
	 and forth loses the ability to cache.

	 Easy lazy grid lines are another good demo of the lazy rendering.
* Log
** Sketchpad D3
...
** [2017-08-23 Wed 12:26]
	 I think it would be a good idea to split the project again. Not so much split
	 as start a series of parallel experiments. Each can just cut and paste out
	 chunks of code.

	 Doing this as a monolith is hurting my ability to track several ideas in
	 parallel. It's also reinforcing bad habits of wholesale refactoring that
	 tends not to work.
** [2017-08-25 Fri 15:19]
	 There are so many intertwinned nuances. The infinite grid data structure, the
	 draw and zoom interactions, the window ns, affine txs, canvas element
	 styling...

	 As it is, these things are not cleanly separated. I don't know how to
	 separate them. This needs a lot more thought. The best way to accomplish it
	 is, I believe, a series of totally separate experiments.

	 Experiment 1)

	 Draw basic shapes. Group them and build more complex shapes with the
	 group. Here zoom and pan are irrelevant.
	 ...
	 Draw a grid on the canvas that is actually a lazy infinite structure.

	 Experiment 4)

	 Mandlebrot set. Infinite zoom and pan.

	 Experiment 5)

	 Masters and instances. Save a picture and reuse it in others.
** [2017-09-08 Fri 09:19]
	 On further thought, these experiments don't get to the heart of the
	 matter. The mandelbrot set in particular isn't important at all, it's just a
	 curious tangent.

	 The basic drawing and animation are important because they cleaned up the
	 conception of what I'm doing. I realise that most of the machinery that I
	 built is solving incidental problems. Also, I now have a working, simplified
	 window pan and zoom system.

*** Important Systems Outstanding
		- Mapping events to objects on the canvas

		- Visual editing of visual objects

		- Recursive affine txs of shapes

		- Master - instance logic

*** Incidental Observations
		canvas-demos does 80% of what infograph does with about a third of the
		code. How is that? It's not just that that last 20% is hard; that particular
		20% isn't important. This needs a bit of analysis.

** [2017-09-12 Tue 10:48]
	 Always keep in mind that the point is to get the computer to eventually
	 generalise the shapes you draw and suggest better ones. Assisted discovery.

	 Recursive shapes defined by nested affine transforms was amazingly simple to
	 implement. Canvas did good job with that. Or maybe it's just the mathematical
	 simplicity of the problem shining through. Still canvas made it trivial to
	 implement (except for not exposing the current transform...).

	 We also now have a generalised master-instance logic where shapes are just
	 built up from compositions of other shapes and modifying transforms. If you
	 swap out the inner shapes, the outer one updates automatically. Of course we
	 still need change tracking and versioning.

	 Both visual editing and code mirroring require more than just a canvas. I've
	 got to decide between using something like codemirror, or implementing my own
	 very basic code editor. I can start with a textarea again for the inital
	 attempt. Why make it more complicated than I need at present?

	 That still leaves the visual editing. I need widgets to do things like select
	 the current constructor, show that a given shape is selected, choose a colour
	 or other style info, etc.. Plus I need a clear and intuitive way to import
	 one image into another and manipulate it. Touch makes most of that far
	 easier. Mostly because two finger touch implicitely chooses an origin for the
	 transformations.

	 So I suppose I should start with visual editing, since I need that before I
	 can generate any code to display.

	 Focus on touch transforms, and deal with control point widgets later
	 on. That's a whole other design task. Not an especially hard one, but in a
	 different vein.

	 Try to keep a clean separation between the core drawing logic and the shape
	 manipulation logic. In particular, don't tie them together with a type system
	 like you did last time.

	 Selection and control widgets will depend on the kind of shape they modify,
	 of course, but those shapes need to be open and closed.

	 Windowing is going to make life difficult. The user is going to have an open
	 number of canvases, and each will potentially need its own window. Actually
	 that's not too bad, I just need to wrap up the logic so that we can import
	 multiple canvases into a single app.

	 No, that's not necessary. It would suffice to keep a list of shape, window
	 pairs and swap them out on a single canvas. Getting rid of windowing on
	 drawings would simplify things, but at the same time it would make it harder
	 to zoom in to correct fine details.
